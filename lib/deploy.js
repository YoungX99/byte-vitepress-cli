...
async function checkCfg(path) {
  ...
  let serviceID = jsonConfig.service_ID;
  let credentials = jsonConfig.credentials;

  if (!serviceID || !credentials) {
    console.log(
      symbols.error,
      chalk.red(
        `1Deploy the docs failed. Please make sure you have entered your service configuration information through the Service Command!`
      )
    );
    process.exit();
  }
...
async function fileProcess(cfgPath, filePath) {
  ...
  if (!exists) {
    ...
    try {
      ...
      console.log(
        symbols.success,
        chalk.cyan(`You have successfully deployed the project with version ${chalk.greenBright(data.version)}!`)
      );
      process.exit();
    } catch (error) {
      console.log(error);
...
/* async function deployAPI(formData, url, headersInfo) {
  axios.defaults.headers = {
    "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
  };
  try {
    let res = await axios.post(url, qs.stringify(formData), {
      headers: {
        Authorization: headersInfo,
      },
    });
